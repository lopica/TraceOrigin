...
function Item() {
    ...
    const { data: itemLogsData, isError: isItemLogsError, isFetching: isItemLogsFetching } = useFetchItemLogsByProductRecognitionQuery(productRecognition);
    ...
    if (isItemLogsFetching) {
        ...
        if (itemLogsData) {
            itemLine = <ul className="timeline w-svw sm:w-[640px] overflow-x-auto overflow-y-hidden h-full">
                {itemLogsData?.itemLogDTOs.map((log, index) => (
                    <li key={index} className='h-full'>
                        ...
                        <div className="timeline-middle">
                            <button className="w-10 h-10 rounded-full bg-accent" onClick={() => setCurrentEvent(log.itemLogId)}></button>
                        </div>
                        <div className="timeline-end timeline-box">{log.partyName}</div>
                        {index !== itemLogsData?.itemLogDTOs?.length - 1 && <hr className="bg-base-content" />}
                    </li>
                ))}
            </ul>
        } else {
