                  ...
                  <span className="ml-2">
                    {
                      priorityOptions.find(
                        (option) => option.value === selectedIssue.priority
                      ).label
                    }
                  </span>
                  <span className="ml-2">
          ...
          <>
            <div className="bg-white p-6 rounded-lg shadow-md mt-4">
              <h3
                ...
                onClick={toggleCauseDetail}
              >
                Nguyên nhân{" "}
                <span>
                  {showCauseDetail ? <FaChevronUp /> : <FaChevronDown />}
                </span>
              </h3>
              {showCauseDetail && (
                <p className="font-medium">{selectedIssue.causeDetail}</p>
              )}
            </div>
            <div className="bg-white p-6 rounded-lg shadow-md mt-4">
              <h3
                className="text-xl font-semibold mb-2 flex justify-between items-center cursor-pointer"
                onClick={toggleResponseDetail}
              >
                Phản hồi{" "}
                <span>
                  {showResponseDetail ? <FaChevronUp /> : <FaChevronDown />}
                </span>
              </h3>
              {showResponseDetail && (
                <p className="font-medium">{selectedIssue.responseDetail}</p>
              )}
            </div>
            <div className="bg-white p-6 rounded-lg shadow-md mt-4">
              <h3
                className="text-xl font-semibold mb-2 flex justify-between items-center cursor-pointer"
                onClick={toggleImageReports}
              >
                Báo cáo hình ảnh{" "}
                <span>
                  {showImageReports ? <FaChevronUp /> : <FaChevronDown />}
                </span>
              </h3>
              {showImageReports && (
            ...
            {showInfo && (
              <>
                ...
                <p className="font-medium flex items-center">
                  <FaUser className="mr-2 text-gray-500" />
                  <div>
                    <span className="font-bold">Báo cáo cho:</span>
                    <br />
